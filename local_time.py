...
def get_time(ques: str):
    ...
    if flag == 1 :        
        ...
        minute = res.tm_min
        sec = res.tm_sec
        ans = "Bây giờ là: " + str(hour) + " giờ," + str(minute) + " phút, " + str(sec) + " giây."
        return ans
    return "false"
...
def get_location(ques: str) : # answer user's location , city, country, longtitude, latitude.
    ...
    if flag == 1 :
        ...
        if cit.lower() == "hanoi":
            cit = " Hà Nội "
        location_ = "Hiện tại, vị trí của bạn đang ở " + cit + " - " + country
        print(location_)
...
def get_day(ques: str):
    ...
    if flag == 1 :        
        ...
        ans = "Hôm nay là " + str(weekday) + ", ngày " + str(day) + " tháng " + str(month) + " năm " +str(year)
